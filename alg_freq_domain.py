...
class Alg_freq_domain():
        ...
        for i in range(len(bpm_interval)):
            ...
            if len(status) == 0 or self.USE_BDT == 0:
                ...
                if len(stable_index_list) == 0:
                    ...
                    elif (self.confidence_level[i] == 1) and self.USE_SVM_reliability:
                        stable_index_list.append(truth_peak_index)
                    truth_peak_index = np.median(np.copy(stable_index_list[(len(stable_index_list) + 1) % 2:]))
                    height = 0.5
    ...
    def get_confidence_level(self, spec_data, peak_index, status):
        ...
        for i in range(len(peak_index)):
            if self.USE_SVM_reliability and status[i] > 2: 
                reliability = self.get_reliability(self.ss_unnorm[:,i], self.nss_unnorm[:,i])
                if reliability > 0.5:
                    result.append(1)
                else:
                    result.append(0)
                list_peak_height.append(reliability)
    ...
    def get_scenario_knn(self, pressure_data, acc_x, acc_y, acc_z):
        ...
        pp = svm.predict(data.iloc[:, [0, 1, 2]])
        data['p3c'] = pp
    ...
    def get_reliability(self,ss,nss):
        ################
        #input : unnormalize ss  and  unnormalize nss
        #
        #output : 1,0
        #
        ################
        ...
        reli_svm = re_svm.predict(x_logs)
        return  reli_svm[0]
    ...
    def get_heart_rate(self, data, g_sensor_x, g_sensor_y, g_sensor_z):
        ...
        self.acc_org_peak_ratio = self.acc_org_peak_ratio > 4

        self.status = self.get_scenario(self.filter_p_sensor_data, self.filter_g_sensor_data, self.acc_org_peak_ratio)
        ...
        for i in range(len(self.bpm)):
            ...
            if len(self.HR_serial_HRV) == self.HRV_t_len:
                ...
                if self.vigor_learning_finish == 0:
                    if MEAN_HR > 0 :
                        self.HRV_reference_vigor.append([MEAN_HR, SDNN, RMSSD])
                        if len(self.HRV_reference_vigor) >= 15:
                            self.vigor_learning_finish = 1
                else:
